	...
	var methods = {
		...
		addOption : function(options) {
			...
			return this.each(function () {
				...
				if (!data)return;
				if (options)$.extend(oAddOption, options);

				var	strEl = "<option value='" + oAddOption.value + "' " + (oAddOption.selected ? "selected" : "") + " >" + oAddOption.name + "</option>";
                    
                var opt = el.find('option[value="'+ oAddOption.value +'"]');
                if( opt.length ){
                   console.warn('option[value='+ oAddOption.value +'] is existed!');
                   return;
                }
				el.append(strEl);
        ...
        removeOption : function( options ){
            var defaults = {name: false,value: false};
            return this.each(function () {
                var el = $(this), data = el.data('multiselect2side');
                
                if(!data)return;
                if (options)$.extend(defaults, options);
                if(!defaults.value)return;
                
                el.find('option[value="'+ defaults.value +'"]').remove();
                // ELEMENTS
                var allSel = el.next().children(".ms2side__select").children("select");
                var leftSel = (data.selectedPosition == 'right') ? allSel.eq(0) : allSel.eq(1);
                var rightSel = (data.selectedPosition == 'right') ? allSel.eq(1) : allSel.eq(0);
                
                var opt = rightSel.find('option[value="'+ defaults.value +'"]');
                if( opt.length ){
                    opt.remove();
                    rightSel.trigger('change');
                }
                opt = leftSel.find('option[value="'+ defaults.value +'"]');
                if( opt.length ){
                    opt.remove();
                    leftSel.trigger('change');
                }
        ...
        getValues : function( options ){
            ...
            var opts = rightSel.find('option');
            $.each(opts, function(i, opt){
                result[result.length] = {'name':opt.innerHTML, 'value' : opt.value };
            });
            return result;
        }
	};
